...
void Mesh::createCube(const char *textureFile, const char *bgTextureFile)
{
    ...
    // Front Face
    std::vector<glm::vec4> pos {glm::vec4(-1, -1, 0.5, 1),
                                glm::vec4(1, -1, 0.5, 1),
                                glm::vec4(1, 1, 0.5, 1),
                                glm::vec4(-1, 1, 0.5, 1)};
    ...
    glm::mat4 translateMat = glm::translate(glm::mat4(), glm::vec3(0.0, 0.0, 0.5));
    glm::mat4 translateMat1 = glm::translate(glm::mat4(), glm::vec3(0.0, 0.0, -0.5));
    for (int i = 1; i < 4; i++) {
        float angle = 90.0*i;
        glm::mat4 rotateMat = glm::rotate(glm::mat4(), angle, glm::vec3(0, 1, 0));
        for (int j = 0; j < 4; j++) {
            pos.push_back(translateMat1*rotateMat*translateMat*pos[j]);
            nor.push_back(rotateMat*nor[j]);
            uvs.push_back(uvs[j]);
        }
    }

    for (int i = 1; i < 3; i++) {
        float angle = 180*i - 90.0;
        glm::mat4 rotateMat = glm::rotate(glm::mat4(), angle, glm::vec3(1, 0, 0));
        for (int j = 0; j < 4; j++) {
            pos.push_back(translateMat1*rotateMat*translateMat*pos[j]);
            nor.push_back(rotateMat*nor[j]);
            uvs.push_back(uvs[j]);
