...
void PostProcessShader::setupMemberVars()
{
    ...
    unifMouse        = context->glGetUniformLocation(prog, "u_Mouse");
...
void PostProcessShader::setMouse(const glm::vec2 &mouse) {
    useMe();

    if (unifMouse != -1) {
        // pass a vec3 uniform variable into shader

        context->glUniform2fv(unifMouse,
                              // How many vector to pass
                              1,
